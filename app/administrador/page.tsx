...
type MatriculadoAdmin = {
  ...
  fecha_ultima_acreditacion: string | null
...
async function crearMatriculado(formData: FormData) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      fecha_ultima_acreditacion: fechaEmision,
...
async function renovarMatriculado(
  ...
  const { error } = await supabase
    ...
    .update({
      /*
       * fecha_emision queda como fecha de alta original.
       * En cada renovación actualizamos la fecha de
       * última acreditación y el nuevo vencimiento.
       */
      fecha_ultima_acreditacion: fechaRenovacion,
      fecha_vencimiento: fechaVencimiento,
    })
    .eq("id", id)
...
export default async function AdministradorPage({
  ...
  if (resultados.length > 0) {
    ...
    const {
      data: documentosPdf,
      error: errorDocumentosPdf,
    } = await supabaseDocumentos
      .from("documentos_pdf_renacli")
      .select(
        "id, matriculado_id, codigo_documento, numero_matricula, estado, fecha_emision, fecha_vencimiento, generado_en, activo"
      )
      .in(
        "matriculado_id",
        idsResultados
    ...
    if (errorDocumentosPdf) {
      console.error(
        "[RENACLI] Error obteniendo documentos PDF:",
        errorDocumentosPdf
      )
    } else {
      for (
        const documento of
        (documentosPdf ?? []) as DocumentoPdfRenacli[]
      ) {
        const actuales =
          documentosPorMatriculado.get(
            documento.matriculado_id
          ) ?? []

        actuales.push(documento)

        documentosPorMatriculado.set(
          documento.matriculado_id,
          actuales
        )
      }
