...
import {
  formatearFecha,
  type MatriculadoPublico,
} from "@/lib/matriculados"
...
export function FichaMatriculado({
  ...
  const iniciales = autorizaPublicacion
    ? `${matriculado.apellido.charAt(0)}${matriculado.nombre.charAt(0)}`.toUpperCase()
    : ""
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      {autorizaPublicacion ? (
        ...
        <div className="border-b border-border bg-secondary/60 p-5 sm:p-6">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
      ...
      <p className="flex items-start gap-2 border-t border-border bg-muted/60 px-5 py-3 text-xs leading-relaxed text-muted-foreground sm:px-6">
        ...
        <span className="text-pretty">
          {autorizaPublicacion
            ...
            : "Esta consulta confirma únicamente la matrícula, su estado y su vigencia. Los datos personales no se muestran sin autorización expresa del titular."}
        </span>
      </p>
    </article>
  )
